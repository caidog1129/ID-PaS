...
def compute_mip_representation(mip_file):
    ...
    for var_ind, (decision_var, relax_var) in enumerate(zip(model.getVars(), relaxation.getVars())):
        ...
        if decision_var.VType == gp.GRB.INTEGER:
            i_vars.append(var_ind)
    ...
    i_vars = torch.as_tensor(i_vars, dtype=torch.int32)

    return A, v_map, variable_features, constraint_features, b_vars, i_vars, objective_coefficients
    ...
    for i in range(sols.shape[0]):
        ...
        if not (solData is None):
            ...
            for entry in neg_ex_priority_list:
                ...
                if "/Hard-BI/" in solFilePath:
                    entry_neg_ex = [[1 if val > 1 else val for val in solData[entry][i][0][k][:2540]]
                            for k in range(len(solData[entry][i][0]))]
                elif "/Very-Hard-BI/" in solFilePath:
                    entry_neg_ex = [[1 if val > 1 else val for val in solData[entry][i][0][k][:25119]]
                            for k in range(len(solData[entry][i][0]))]
...
class GraphDataset(torch_geometric.data.Dataset):
    ...
    def _load_graph(self, index):
        ...
        A, v_map, v_nodes, c_nodes, b_vars, i_vars,_ = compute_mip_representation(inspath)
        ...
        # Identity-aware
        N_v = variable_features.size(0)
        graph.variable_ids = torch.arange(N_v, dtype=torch.long)  # 0..N_v-1, same across all instances
        ...
        graph.negsamples = torch.FloatTensor(negative_samples).reshape(-1)
        print(negative_samples.shape)

        return graph

class GraphDataset_id_v1(torch_geometric.data.Dataset):
    """
    This class encodes a collection of graphs, as well as a method to load such graphs from the disk.
    It can be used in turn by the data loaders provided by pytorch geometric.
    """
    ...
    def _load_graph(self, index):
        ...
        A, v_map, v_nodes, c_nodes, b_vars, i_vars, objective_coefficients =compute_mip_representation(inspath)
...
class GraphDataset_id_v2(GraphDataset):
    def __init__(self, sample_files, input_args):
        super().__init__(sample_files, input_args)

    def _load_graph(self, index):
        graph = super()._load_graph(index)
        n_vars = graph.variable_features.size(0)
        bin_feats, B = binary_positional_features(n_vars)
        graph.variable_features = torch.cat([graph.variable_features, bin_feats], dim=1)
        return graph